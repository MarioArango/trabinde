...
administradorController.listar_trabajadores = (req, res) => {
    ...
    mysql.query(sql, (error, data) => {
        if (!error) {
            ...
            res.status(400).send({ status: "Error", message: "No se logro obtener los trabajadores", code: 400 });
        }
    })   
};
...
administradorController.listar_solicitantes = (req, res) => {
    ...
    mysql.query(sql, (error, data) => {
        if (!error) {
            ...
            res.status(400).send({ status: "Error", message: "No se logro obtener los solicitantes", code: 400 });
        }
    })   
};
